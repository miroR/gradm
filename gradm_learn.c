...
static void sort_names(struct learn_info ** obj_array, unsigned long num)
{
	return qsort(obj_array, num, sizeof(struct learn_info *), strcompare);
}

static int has_similar_dir(const char * f1, const char * f2, unsigned int len)
{
...
static void set_common_dir(const char * fname, char ** common_dir, unsigned int * len)
{
	unsigned int i;
	char * p;
...
static int have_common_dir(const char * fname, 
				char * common_dir, unsigned int len)
{
	unsigned int i;
...
static void remove_queue_items(void)
{
	unsigned long i;
...
static void add_to_remove_queue(unsigned long offset)
{
	if(!learn_queue)
		learn_queue = (unsigned long *) calloc(1, sizeof(unsigned long));

	if(!learn_queue)
		failure("calloc");

	learn_queue = realloc(learn_queue, (learn_queue_size + 1) * sizeof(unsigned long));	
...
static void clear_remove_queue(void)
{
	learn_queue_size = 0;
	learn_queue = NULL;
	return;
			...
			__u16 mode)
{
	...
	*tmp_db = (struct ip_learn_info *) calloc(1, sizeof(struct ip_learn_info));
			...
			char * obj_name, __u32 mode)
{
	...
	if((learn_db = (struct learn_info **) realloc(learn_db, (learn_num + 1) * sizeof(struct learn_info *))) == NULL)
		failure("calloc");
			...
			if(!strcmp((*(learn_db + i))->subjname, proc->filename) && 
			   ...
			   ((*(learn_db + i))->roletype == role->roletype)) { 
				...
				if(!strlen((*(learn_db + i))->obj_name) && !((*(learn_db + i))->res_cur) && !((*(learn_db + i))->res_max))
					modify_caps(proc, (*(learn_db + i))->mode);
				else if(!strlen((*(learn_db + i))->obj_name))
					modify_res(proc, (*(learn_db + i))->mode, (*(learn_db + i))->res_cur, (*(learn_db + i))->res_max);
				else
		...
		for(i = 0; i < ip_learn_num; i++) {
			if(!strcmp((*(ip_learn_db + i))->subjname, proc->filename) && 
			   ...
			   ((*(learn_db + i))->roletype == role->roletype)) { 
				...
				tmp_ip.addr = (*(ip_learn_db + i))->addr;
				tmp_ip.low = (*(ip_learn_db + i))->port;
				tmp_ip.high = (*(ip_learn_db + i))->port;

				tmp_ip.netmask = 0xffffffff;
				tmp_ip.type |= (1 << (*(ip_learn_db + i))->sock);
				tmp_ip.proto[(*(ip_learn_db + i))->proto / 32] |= (1 << ((*(ip_learn_db + i))->proto % 32));
				tmp_ip.mode = (*(ip_learn_db + i))->mode;

				if((*(ip_learn_db + i))->mode == GR_IP_BIND)
					add_ip_acl(proc, GR_IP_BIND, &tmp_ip);
				else if((*(ip_learn_db + i))->mode == GR_IP_CONNECT)
					add_ip_acl(proc, GR_IP_CONNECT, &tmp_ip);
			}
		}
	}
	}
...
void handle_learn_logs (const char * file, FILE * stream)
{
