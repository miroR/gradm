...
static void show_help(void)
{
	printf ("gradm %s\n"
		...
		"	-a, --admin	Provides full overriding access for the current user\n"
...
static void conv_name_to_num(const char * filename, unsigned short * dev, ino_t * inode)
{
	struct stat fstat;
...
void parse_args(int argc, char *argv[])
{
	...
	struct gr_arg * grarg;
	const char * const short_opts = "EDPRL::O:M:ahv";
	...
	while((next_option = getopt_long(argc, argv, short_opts, long_opts, NULL)) != -1) {
		...
		switch(next_option) {
			...
			case 'R':
				...
				break;
			case 'M':
				if((argc != 3) || (optind > argc) || (strlen(optarg) < 1))
					show_help();
				...
				if(isdigit(optarg[0]))
					...
					conv_name_to_num(optarg, &entry.segv_dev, &entry.segv_inode);
				grarg = conv_user_to_kernel(&entry);
				transmit_to_kernel(grarg, sizeof(struct gr_arg));
				memset(grarg, 0, sizeof(struct gr_arg));
				exit(EXIT_SUCCESS);
