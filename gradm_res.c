...
static unsigned short name_to_res(const char * name)
{
	int i;
...
static unsigned short res_to_mask(unsigned short res)
{
	return (1 << res);
}

static unsigned long conv_res(const char * lim)
{
	unsigned long res;
	char * p;
	int i;
...
void modify_res(struct proc_acl * proc, int res, unsigned long cur, unsigned long max)
{
	if((res < 0) || (res > (sizeof(rlim_table) / sizeof(struct rlimconv))))
		return;
